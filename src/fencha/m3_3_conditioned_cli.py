...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fencha-m3-3-conditioned",
        description=(
            "Explore hierarchical total-exit risk with path models trained only "
            "on observations where an exit actually occurred"
        ),
    )
    parser.add_argument(
        "--cases",
    ...
    parser.add_argument(
        "--output",
        default="data/processed/m3_3_conditioned_competing_benchmark.json",
    )
    return parser
...
def main() -> None:
    ...
    raw_path_model = AnalogForecaster(
    ...
    conditioned_path_model = ExitConditionedPathForecaster(raw_path_model)
    hybrid = HybridCompetingForecaster(
        total_model=total_model,
        mechanism_model=conditioned_path_model,
    )
    ...
    print(
        ...
        f"path_accuracy={report.adjusted_conditional_accuracy:.6f} "
        f"output={output}"
    )
