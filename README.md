...
- **GDELT 新闻信号试验**：确定性周采样全球事件流，构造抗议、冲突、合作、语调和报道强度的 30/90 天历史窗口。
...
M2 不下载 TB 级完整火hose，而采用冻结的确定性周采样协议：每七天固定抓取一个全球事件切片，并记录 URL、时间、文件大小和 SHA-256。每个国家会得到过去 30 天和 90 天的抗议、言语冲突、物质冲突、合作、负面语调、平均语调、Goldstein 强度及报道量特征。

```bash
python -m fencha.m2_cli build \
  --start 2020-01-05 \
  --end 2023-01-01

python -m fencha.m2_cli compare \
  --holdout-start 2022-01-01
```

比较报告让以下三套方法在完全相同的目标月份上考试：

1. 平滑历史基础率；
2. 只使用制度和任期特征的历史类比模型；
3. 制度结构加 GDELT 新闻信号的历史类比模型。

详见 [`docs/M2_GDELT_PILOT.md`](docs/M2_GDELT_PILOT.md)。
...
2. **GDELT**：全球事件、行动者、地点、事件类型与媒体语调，M2 周采样试验已经进入代码。
...
ParlGov 首版只覆盖其收录的 established democracies。M2 的周采样只代表可复现的 GDELT 子样本，不等于完整新闻世界；小国可能出现稀疏信号，媒体覆盖变化也可能影响报道量。真实预测价值必须由冻结时间盲测证明，不能因为加入了更多数据就预设模型一定更强。
